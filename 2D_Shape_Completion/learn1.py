...
agent = PPO("MlpPolicy", env, verbose=1, device=device, n_steps=2048, batch_size=64, n_epochs=1)

# Create the callback to track the running average of rewards
callback = RunningRewardCallback(window_size=10)

agent.learn(total_timesteps=1, callback=callback, reset_num_timesteps=False)